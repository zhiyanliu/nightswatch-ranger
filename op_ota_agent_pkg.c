...
static int parse_job_doc(pjob pj, char *pkg_url, size_t pkg_url_l, unsigned char *pkg_md5, size_t pkg_md5_l,
        char *agent_var, size_t agent_ver_l) {
    ...
    rc = parseStringValue((char*)agent_var, agent_ver_l, pj->job_doc, tok_agent_ver);
    if (SUCCESS != rc) {
...
static int step1_download_pkg_file(pjob_dispatch_param pparam, char *alter_par_path, size_t alter_par_path_l,
        ...
        unsigned char *pkg_md5_dst, size_t pkg_md5_l, char *agent_var, size_t agent_var_l) {
    ...
    rc = parse_job_doc(pparam->pj, pkg_url, 4096, pkg_md5_dst, pkg_md5_l, agent_var, agent_var_l);
    if (0 != rc) {
...
static int step5_restart_agent(pjob_dispatch_param pparam, char *alter_par_name, char *agent_var) {
    int rc = 0;
    ...
    rc = execl(agent_path, agent_path, "--ota_agent_pkg_job_id", pparam->pj->job_id,
            "--ota_agent_pkg_par_name", alter_par_name, "--ota_agent_ver_name", agent_var, NULL);
    if (0 != rc) {
...
int op_ota_agent_pkg_entry(pjob_dispatch_param pparam) {
    char alter_par_path[PATH_MAX + 1], alter_par_name[PATH_MAX + 1],
            alter_agent_pkg_file_path[PATH_MAX + 1], agent_var[100];
    ...
    rc = step1_download_pkg_file(pparam, alter_par_path, PATH_MAX + 1, alter_agent_pkg_file_path, PATH_MAX + 1,
            pkg_md5_dst, MD5_SUM_LENGTH + 1, agent_var, 100);
    if (0 != rc) {
    ...
    rc = step5_restart_agent(pparam, alter_par_name, agent_var);
    if (0 != rc) {
        dmp_dev_client_job_failed(pparam->paws_iot_client, pparam->thing_name, pparam->pj->job_id,
